...
def _crc32c(data: bytes) -> int:
  """Returns the CRC32C checksum of `data`."""
...
class _GcpKmsAead(aead.Aead):
  ...
  def decrypt(self, ciphertext: bytes, associated_data: bytes) -> bytes:
    ...
    Verifies request and response integrity per Cloud KMS Data Integrity
    Guidelines: sets request CRC32C fields, then fails closed if the returned
    plaintext CRC32C does not match the locally recomputed checksum. Cloud KMS
    reports request checksum failures as RPC errors; DecryptResponse does not
    include request-verification fields.
    """
