...
def _get_required_str(key: Dict[str, Union[str, List[str]]], name: str) -> str:
  """Reads key[name], rejecting non-string types with tink.TinkError.

  Args:
    key: The JWK object (a JSON object represented as Python dict).
    name: The name of the field to read.

  Returns:
    The value of key[name] if it is a string.

  Raises:
    TinkError if key[name] is not a string or does not exist.
...
def _get_optional_str(
    key: Dict[str, Union[str, List[str]]], name: str
) -> Optional[str]:
...
def _reject_duplicate_keys(pairs: List[Tuple[Any, Any]]):
  """object_pairs_hook for json.loads that rejects duplicate JSON keys.

  Args:
    pairs: The list of (key, value) pairs in the JSON object.

  Returns:
    A dict of the JSON object.

  Raises:
    TinkError if the JSON object has duplicate keys.
